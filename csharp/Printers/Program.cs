...
namespace Printers
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int selectedPrinterNumber = 0;

            bool isEnteredValueNumber = false;
            bool isPrintersExists = false;
            while (!isEnteredValueNumber || !isPrintersExists)
            {
                isEnteredValueNumber = Int32.TryParse(Console.ReadLine(), out selectedPrinterNumber);
            ...
            switch (selectedEncoder)
            {
                case GIAI_MENU_ITEM_NUMBER:
                    ...
                    AddTextElement(label, TextParams.X, TextParams.Y, TextParams.WIDTH, TextParams.HEIGHT, 
                            TextParams.FONT_FAMILY, TextParams.FONT_SIZE, "EpcGiai96Encoder");

                    AddBarcode(label, BarcoreParams.X, BarcoreParams.Y, BarcoreParams.WIDTH, BarcoreParams.HEIGHT, 
                            BarcoreParams.FONT_SIZE, COMPANY_PREFIX.ToString(CultureInfo.InvariantCulture) + 
                ...
                case SGTIN_MENU_ITEM_NUMBER:
                    ...
                    AddTextElement(label, TextParams.X, TextParams.Y, TextParams.WIDTH, TextParams.HEIGHT,
                            TextParams.FONT_FAMILY, TextParams.FONT_SIZE, "EpcSgtin96Encoder");
                ...
                case SSCC_MENU_ITEM_NUMBER:
                    ...
                    AddTextElement(label, TextParams.X, TextParams.Y, TextParams.WIDTH, TextParams.HEIGHT,
                            TextParams.FONT_FAMILY, TextParams.FONT_SIZE, "EpcSscc96Encoder");
                ...
                case WRITE_MULTIPLE_BLOCKS_LABEL_ELEMENT:                   
                    AddTextElement(label, TextParams.X, TextParams.Y, TextParams.WIDTH, TextParams.HEIGHT,
                            TextParams.FONT_FAMILY, TextParams.FONT_SIZE, "Write custom data");

                    var data = new byte[] { 255, 255, 255, 255, 255 };
                    AddEpcElement(label, data, START_BLOCK, data.Length, TagMemoryBankEnum.Epc);
            ...
            AddLineElement(label, ImageParams.X, ImageParams.Y + logo.Height + 1, label.Width, ImageParams.Y + logo.Height + 1, color: Color.FromRgb(150,150,150));
        ...
        private static void AddGiaiEpcElement(PrintLabel label, 
                ...
                ulong asset)
        {
            var encoder = new EpcGiai96Encoder(companyPrefix, asset);
        ...
        private static void AddSsccEpcElement(PrintLabel label, 
                ...
                ulong extension)
        {
            var encoder = new EpcSscc96Encoder(companyPrefix, extension);
            var writeEpcElement = new WriteMultipleBlocksLabelElement(encoder.Epc);
            label.Elements.Add(writeEpcElement);
        }
